...
const[sender_info,setSenderInfo]=useState([])
  ...
  return (
    ...
    {/* {!isMobile &&( */}
      ...
      {isMobile &&(
        <div onClick={()=>navigation('/message')} style={{marginLeft:"21px" }}>
      <FontAwesomeIcon icon={faArrowLeft} style={{ color:  "black" }} className="iconstyle" />
      </div>
      )}
   ...
   <Messageshow  own={item.sender === userlogin.message_id ? true : false} message={item.messagetext} time={item.updatedAt
              } />
    ))}
   
  <AutoGrowTextarea  id={id} userId={userlogin.message_id} setmessagedata={setmessagedata} messagedata={messagedata} setMessagestatus={setMessagestatus} Onsubmit={handleSend} messageref={messageref} setMessage={setMessage}/>
    </div>
