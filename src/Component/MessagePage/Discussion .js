...
export default function Discussion({ photo, name, message, timer, isMobile }) {
  const userId = useSelector(state => state.myReducer.data.user_id)
  ...
  return (
    ...
    <div className={`${"discussions"}`}>
      ...
      <div className="discussion" key={item._id} onClick={()=>handleMesage(item._id,item.members?.filter((item) => item !== userId).toString())} style={{cursor:"pointer"}}>
        <div className="photo" style={backgroundImageStyle}>
          <div className="online"></div>
        </div>
        <div className="desc-contact">
          <p className="name">{userProfiles[item.members?.find((item) => item !== userId)]?.name}</p>
